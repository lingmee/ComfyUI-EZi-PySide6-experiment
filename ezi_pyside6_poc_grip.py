...
Dragging works ONLY from the designated drag grip in the HTML bar.
Buttons are fully clickable. Resize borders work. Aero Snap works.
"""
...
# ---------------------------------------------------------------------------
# 3. HTML Test Page (drag grip only — buttons are free)
# ---------------------------------------------------------------------------
...
<div id="bar">
  ...
  <!-- Drag grip: only this element initiates window drag -->
  <div class="flex-spacer"></div>
  <div id="drag-grip" onmousedown="startWindowDrag()" title="Drag to move window">&#9776;</div>
  <div class="flex-spacer"></div>
...
<div id="content">
  <div style="text-align:center">
    ...
    <div style="color:#8b949e;margin-bottom:24px">Drag the &#9776; grip to move. Buttons are clickable.</div>
...
class QtBridge(QObject):
    ...
    @Slot()
    def startDrag(self):
        """Initiate native window drag from HTML drag grip."""
        try:
...
class MainWindow(QMainWindow):
    ...
    def _inject_webchannel_scripts(self):
        ...
        (function() {
            ...
            new QWebChannel(qt.webChannelTransport, function(channel) {
                ...
                var result = document.getElementById('result');
                if (result) result.textContent = "Qt bridge connected! Drag \u2630 to move.";
            });
        })();
        """
    ...
    def nativeEvent(self, eventType, message):
        ...
        try:
            ...
            # WM_NCCALCSIZE = 0x83 — suppress native frame drawing
            if msg.message == 0x83:
                if msg.wParam:
                    ...
                    # else: not maximized — client area already equals window area
                    return True, 0
                else:
                    return True, 0

            # WM_NCHITTEST = 0x84 — ONLY edges/corners for resize.
            # We intentionally do NOT return HTCAPTION here because that
            # would steal clicks from HTML buttons in the top bar.
            # Drag is handled by the bridge startDrag() from the HTML grip.
            if msg.message == 0x84:
                ...
                # Everything else: pass through to WebEngine (HTCLIENT)
                return False, 0
...
def main():
    print("=" * 60)
    print("ComfyUI-EZi - PySide6 Web Chrome + Drag + Resize + Snap")
    print("Buttons are clickable. Only the drag grip moves the window.")
    print("=" * 60)
    ...
    print(f"\n[Main] Window shown.")
    print(f"  - Click buttons in the HTML bar -> they work! (no drag steal)")
    print(f"  - Click+drag the \u2630 grip in the bar -> native drag + FancyZones")
    print(f"  - Hover window EDGES / CORNERS -> resize cursor")
    print(f"  - Win+Left/Right/Up/Down -> Aero Snap")
