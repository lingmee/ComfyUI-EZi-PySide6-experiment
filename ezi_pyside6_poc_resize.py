"""
ComfyUI-EZi - PySide6 Proof of Concept (WEB CHROME + DRAG + RESIZE)
...
<head>
<meta charset="UTF-8">
<title>ComfyUI-EZi - PySide6 Web Chrome + Drag + Resize</title>
<style>
...
<div id="bar">
  <div id="dot"></div>
  <span id="status">Web Chrome + Drag + Resize</span>
...
<div id="content">
  <div style="text-align:center">
    ...
    <div style="color:#f1fa8c;font-size:16px;margin-bottom:8px">Web Chrome + Native Drag + Resize</div>
    <div style="color:#8b949e;margin-bottom:24px">Drag the empty bar space to move. Hover edges to resize.</div>
    ...
    <div class="hint">Try dragging window to a FancyZone. Hover near edges/corners for resize cursor.</div>
  </div>
</div>
<script>
...
# ---------------------------------------------------------------------------
# 5. Main Window (NO native title bar — web chrome + resize borders)
# ---------------------------------------------------------------------------
...
class MainWindow(QMainWindow):
    ...
    def _inject_webchannel_scripts(self):
        ...
        (function() {
            ...
            new QWebChannel(qt.webChannelTransport, function(channel) {
                ...
                var result = document.getElementById('result');
                if (result) result.textContent = "Qt bridge connected! Drag bar to move. Hover edges to resize.";
            });
        })();
        """
    ...
    # -----------------------------------------------------------------------
    # Native Windows event handling for resize borders + drag
    # -----------------------------------------------------------------------

    def nativeEvent(self, eventType, message):
        """Handle Windows native messages for frameless resize borders."""
        ...
        try:
            ...
            # WM_NCHITTEST = 0x84 — Windows asks "what is under the cursor?"
            if msg.message == 0x84:
                # Cursor position is in screen coordinates (signed shorts)
                ...
                # Everything else: let Qt / WebEngine handle it (HTCLIENT)
                # We intentionally do NOT return HTCAPTION here because it would
                # steal clicks from HTML buttons. Drag is handled by startDrag().
                return False, 0

            # WM_NCCALCSIZE = 0x83 — prevent Windows from adding native frame borders
            if msg.message == 0x83:
                # Return 0 to keep our custom frameless look without extra borders
                return True, 0
...
def main():
    print("=" * 60)
    print("ComfyUI-EZi - PySide6 Web Chrome + Drag + Resize Test")
    print("=" * 60)
    ...
    print(f"\n[Main] Window shown with WEB CHROME + DRAG + RESIZE.")
    ...
    print(f"  - Hover near window EDGES / CORNERS -> resize cursor appears.")
    print(f"  - Drag edges/corners -> window resizes natively.")
    print(f"  - Drag window to screen edge / FancyZone -> snaps natively.")
